...
async def query(request: QueryRequest, raw_request: Request):
    ...
    try:
        ...
        if request.id.startswith("rag_"):
            ...
            prompt = SYSTEM_PROMPT_CONSTRUCCION.format(question=request.question, language=request.language)
            logger.info("Respuesta de construcción solicitada.")

        elif request.id == "out_of_scope":
            prompt = SYSTEM_PROMPT_OUT_OF_SCOPE.format(question=request.question, language=request.language)
